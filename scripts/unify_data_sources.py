...
def derive_figure_3_data(df):
    ...
    # 2. Edges (Spearman Correlation - S2 treatment only)
    # Filter to S2 only: correlations should reflect stress response, not mix control with stress
    # (mixing treatments creates misleading correlations, e.g. ABA↔stomatal conductance appearing positive)
    df_s2 = df[df['Treatment'] == 'S2']
    print(f"   Filtering for S2 treatment: {len(df_s2)} rows (from {len(df)} total)")
    data_for_corr = df_s2[params].dropna(how='all')
